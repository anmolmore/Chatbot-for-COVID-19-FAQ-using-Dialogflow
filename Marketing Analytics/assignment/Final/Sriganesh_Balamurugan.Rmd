...
#use two clusters
conjoint_data_cluster <- kmeans(conjoint_data, 2)$cluster
demographics_data$Cluster <- conjoint_data_cluster
...
supervised learning technique to classify data
...
plot pyramid graph to see what are most important attributes for customers in each cluster.

Important observations - 

People in cluster 2 prefer - Japanese & European. Brown label, mild and very mild beers

People in cluster 1 prefer - Brown painted, Cannadian beers

```{r}
#copy conjoint data to new dataframe
cd = conjoint_data

#assign clusters calculated
cd$Cluster = conjoint_data_cluster

dd = cbind(cd[22], stack(cd[-22]))
dd$Cluster = as.factor(dd$Cluster)

pyramidGH <- ggplot(dd, aes(x = ind, y = values, fill = Cluster)) + 
  geom_bar(data = subset(dd, Cluster == 1), stat = "identity") + 
  geom_bar(data = subset(dd, Cluster == 2), stat = "identity") + 
  coord_flip()
pyramidGH
```
